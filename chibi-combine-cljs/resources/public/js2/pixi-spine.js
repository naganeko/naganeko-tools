        ...
        if (time < frames[0]) {
          switch (blend) {
            case MixBlend.setup:
              ...
              return;
            case MixBlend.first:
              var light = slot.color, dark = slot.darkColor, setupLight = slot.data.color,
...
(function (pixi_spine) {
  ...
  (function (core) {
    var Bone = (function () {
      ...
      Bone.prototype.w2lr = function (wr) {
        return this.worldToLocalRotation(wr);
      };
      Bone.prototype.l2wr = function (wr) {
        return this.localToWorldRotation(wr);
      };
      Bone.prototype.wr = function () {
        return this.getWorldRotationX();
      };
      // Bone.prototype.w2l = function (wr) {
      //   return this.worldToLocal(wr);
      // };
      // Bone.prototype.l2w = function (wr) {
      //   return this.localToWorld(wr);
      // };
  ...
  (function (core) {
    var Skeleton = (function () {
      ...
      Skeleton.prototype.updateCache = function () {
        ...
        var ikCount = ikConstraints.length, transformCount = transformConstraints.length,
...
//        console.log("SKEL PARSER");
        ...
        console.log("hash=", skeletonData.hash);
        skeletonData.version = input.readString();
        console.log("version=", skeletonData.version);
        ...
        console.log(skeletonData.version);
...

        {
          ...
          // Spine3.7 binary reader 가 없음.
          // pixi-spine-3.7.js 은 SkeletonBinary클래스가 없다
          ...
          if (v == "3.7" || v.startsWith("3.7.")) {
            goto_s3g();
            // throw "not 3.8";
            is38 = false;
          } else
          /*
          https://github.com/EsotericSoftware/spine-runtimes/blob/3.7/spine-libgdx/spine-libgdx/src/com/esotericsoftware/spine/SkeletonBinary.java

          * */
          if (v != "3.8" && !v.startsWith("3.8.")) {
            goto_s3g();
            throw "not 3.8";
          }
        }
        ...
        if (nonessential) {
          ...
          console.log("fps=", skeletonData.fps);
          console.log("imagesPath=", skeletonData.imagesPath);
          console.log("audioPath=", skeletonData.audioPath);
        }
        var n = 0;
        // bones
        if (is38) {
        ...
        console.log("read bones is38=", is38);
        n = input.readInt(true);
        for (var i = 0; i < n; i++) {
          var name_2 = input.readString();
          console.log("bone name=", name_2);
        ...
        // Slots
        n = input.readInt(true);
        console.log("read Slots.. n=", n);
        for (var i = 0; i < n; i++) {
          var slotName = input.readString();
          let boneIdx = input.readInt(true);
          console.log("slot name=", i, slotName, "boneIdx=",boneIdx);
          var boneData = skeletonData.bones[boneIdx];
          var data = new core.SlotData(i, slotName, boneData);
          core.Color.rgba8888ToColor(data.color, input.readInt32());
          var darkColor = input.readInt32();
          if (darkColor != -1)
            core.Color.rgb888ToColor(data.darkColor = new core.Color(), darkColor);
          data.attachmentName = is38
            ? input.readStringRef()
            : input.readString()
          ;
        ...
        // IK constraints.
        console.log("read IK constraints..");
        n = input.readInt(true);
        ...
        console.log("read Transform constraints..");
        n = input.readInt(true);
        for (var i = 0, nn = void 0; i < n; i++) {
        ...
        // Path constraints.
        console.log("read Path constraints..");
        n = input.readInt(true);
        for (var i = 0, nn = void 0; i < n; i++) {
        ...
        // Default skin.
        console.log("read Default skin..");
        var defaultSkin;
        if (is38) {
          defaultSkin = this.readSkin(input, skeletonData, true, nonessential);
        } else {
          // defaultSkin = this.readSkin37(input, skeletonData, "default", nonessential);
          defaultSkin = this.readSkin37(input, skeletonData, true, nonessential);
        }
        if (defaultSkin != null) {
        ...
        { // Skins.
          // console.log("read Skins..");
          var i = skeletonData.skins.length;
          core.Utils.setArraySize(skeletonData.skins, n = i + input.readInt(true));
          console.log("read Skins.. len=", i, "n=" + n);
          if (is38) {
            for (; i < n; i++)
              skeletonData.skins[i] = this.readSkin(input, skeletonData, false, nonessential);
          } else {
            for (; i < n; i++)
              skeletonData.skins[i] = this.readSkin37(input, skeletonData, false, nonessential);
          }
        }
        // Linked meshes.
        console.log("read Linked meshes..");
        ...
        n = input.readInt(true);
        for (var i = 0; i < n; i++) {
          var data = new core.EventData(
            // input.readStringRef()
          is38 ? input.readStringRef() : input.readString()
          );
        ...
        // Animations.
        n = input.readInt(true);
        console.log("read Animations.. n=", n);
        for (var i = 0; i < n; i++) {
            let name = input.readString();
            // if (!name) break;
            let animation = this.readAnimation(input, name, skeletonData);
            if (animation)
               skeletonData.animations.push(animation);
        }
        return skeletonData;
      };
      SkeletonBinary.prototype.readSkin37 = function (input, skeletonData, defaultSkin, nonessential) {
        var skin = null;
        var slotCount = 0;
        // skin = new core.Skin("default");
        var name = defaultSkin ? "default" : input.readString();
        // console.log("skinName=", name);
        skin = new core.Skin(name);
        slotCount = input.readInt(true);
        // console.log("slotCount=", slotCount);
        if (slotCount == 0)
          return null;
        // skin = new core.Skin(skinName);
      ...
      SkeletonBinary.prototype.readAttachment = function (input, skeletonData, skin, slotIndex, attachmentName, nonessential) {
        var scale = this.scale;
        var name = is38 ? input.readStringRef() : input.readString();
        if (name == null)
          name = attachmentName;
        ...
        switch (type) {
          ...
          case core.AttachmentType.Mesh: {
            var path = is38 ? input.readStringRef() : input.readString();
            var color = input.readInt32();
            var vertexCount = input.readInt(true);
          ...
          case core.AttachmentType.LinkedMesh: {
            var path = is38 ? input.readStringRef() : input.readString();
            var color = input.readInt32();
            var skinName = is38 ? input.readStringRef() : input.readString();
            var parent_4 = is38 ? input.readStringRef() : input.readString();
      ...
      SkeletonBinary.prototype.readAnimation = function (input, name, skeletonData) {
        ...
        var n = input.readInt(true);
        console.log("read Slot timelines.. n=", n);
        for (var i = 0; i < n; i++) {
          var slotIndex = input.readInt(true);
          for (var ii = 0, nn = input.readInt(true); ii < nn; ii++) {
        ...
        n = input.readInt(true);
        console.log("read Bone timelines.. n=", n);
        for (var i = 0; i < n; i++) {
        ...
        n = input.readInt(true);
        console.log("read Transfrom constraint timelines.. n=", n);
        for (var i = 0; i < n; i++) {
          var index = input.readInt(true);
          var frameCount = input.readInt(true);
          console.log("index frameCount", index, frameCount);
        ...
        n = input.readInt(true);
        console.log("read Path constraint timelines.. n=", n);
        for (var i = 0; i < n; i++) {
          var index = input.readInt(true);
          ...
          for (var ii = 0, nn = input.readInt(true); ii < nn; ii++) {
            var timelineType = input.readByte();
            var frameCount = input.readInt(true);
            console.log("type= count=", timelineType, frameCount);
            switch (timelineType) {
              case SkeletonBinary.PATH_POSITION: // 0
        ...
        console.log("read Deform timelines..");
        for (var i = 0, n = input.readInt(true); i < n; i++) {
          var skin = skeletonData.skins[input.readInt(true)];
          for (var ii = 0, nn = input.readInt(true); ii < nn; ii++) {
            var slotIndex = input.readInt(true);
            for (var iii = 0, nnn = input.readInt(true); iii < nnn; iii++) {
              var attachment = skin.getAttachment(slotIndex,
                // input.readStringRef()
              is38 ? input.readStringRef() : input.readString()
              );
        ...
        console.log("read DrawOrder timelines..");
        ...
        console.log("read Event timelines..");
  ...
  (function (core) {
    var SkeletonClipping = (function () {
      ...
      SkeletonClipping.makeClockwise = function (polygon) {
        ...
        var area = vertices[verticeslength - 2] * vertices[1] - vertices[0] * vertices[verticeslength - 1], p1x = 0,
          p1y = 0, p2x = 0, p2y = 0;
...
//        console.log("JSON PARSER");
        ...
        if (skeletonMap != null) {
          ...
          console.log("skeletonData.version", skeletonData.version);

          if (skeletonData.version.substr(0, 3) === '3.7') {
            var error = "PixiJS Spine plugin supports only format for Spine 3.8. " +
              "Your model has version " + skeletonMap.spine + ". Please look in pixi-spine repository README for another branch.";
            console.error(error);
            alert(error);
            throw "not 3.8";
            return null;
          } else if (skeletonData.version.substr(0, 3) !== '3.8') {
      ...
      SkeletonJson.prototype.readCurve = function (map, timeline, frameIndex) {
        ...
        if (map.curve === "stepped") {
          timeline.setStepped(frameIndex);
        } else if (map.curve === "linear") {
          // DO NOTHING. 일부 json은 이거 들어가 있음.
        } else {
          var curve = map.curve;
...
(function (pixi_spine) {
  ...
  (function (core) {
    ...
    // public 클래스
    core.TextureAtlasReader = TextureAtlasReader;
