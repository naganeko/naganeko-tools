...
(defn draw-bones [doll]
  ...
  (let [bones-cntr (? doll._chibi_psd.ghs.bones)
    ...
    ;(.log js/console "bones" bones)

    (mapv
      (fn [idx bone]
        (let [blen (or (? bone.data.length) 1)
              bwx (aget bone "worldX")
              bwy (aget bone "worldY")
              ;bwy (? bone.worldY)
              sx (+ sk-x bwx)
              sy (+ sk-y bwy)
              ;ex (+ sk-x (* blen (.-m00 bone)) bwx)
              ex (+ sk-x (* blen (? bone.matrix.a)) bwx)
              ;ey (+ sk-y (* blen (.-m10 bone)) bwy)
              ; FIXME b or c 어디가 정답인가???
              ey (+ sk-y (* blen (? bone.matrix.c)) bwy)
              w (Math/abs (- sx ex))
              h (Math/abs (- sy ey))
              a2 (Math/pow w 2)
              b h
