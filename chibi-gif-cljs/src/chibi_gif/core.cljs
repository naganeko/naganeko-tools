(ns chibi-gif.core
  ...
  (:require
    ...
    [renden.chibi :as c]
    [renden.skb3 :as skb]
    [chibi-gif.features :as features]
    [renden.chibi-debug :as chb-debug]

    [chibi-gif.experimentals :as experimentals]
...
(defn get-auto-update []
  (if
    (nil? js/PIXI.spine.Spine.globalAutoUpdate)
    (? js/PIXI.spine.Spine.globalAutoUpdate)
    (? js/PIXI.spine.settings.GLOBAL_AUTO_UPDATE)
    ))

(defn set-auto-update [flag]
  (! js/PIXI.spine.Spine.globalAutoUpdate flag)
  (! js/PIXI.spine.settings.GLOBAL_AUTO_UPDATE flag)
  )
...
(defn gen-psd3 [doll shot-bg psd-children d-bs-x d-bs-y d-bs-w d-bs-h]
  (let [bones-cntr (chb-debug/draw-bones doll)
        layers #js []
        bones-layer #js {
                         :blendMode "normal"
    ...
    ;(.log js/console "bones-cntr"  bones-cntr)
...
(defn gen-psd2 [doll nr hs d-bs-x d-bs-y d-bs-w d-bs-h]
  (let [
        ...
        skin (aget doll "spineData" "defaultSkin")
        ...
        region-class (if (= "3.8" js/window.sp_runtime_version)
                       js/PIXI.spine.RegionAttachment
                       js/PIXI.spine.core.RegionAttachment
                       )
        mesh-attch-class (if (= "3.8" js/window.sp_runtime_version)
                       js/PIXI.spine.MeshAttachment
                       js/PIXI.spine.core.MeshAttachment
                       )

        ]
    ;(.log js/console slot-idx-att-lst)
    ;(doseq [idx (keys slot-idx-att-lst)]
    (doseq [idx (range (count slot-idx-att-lst))]
      ;(.log js/console idx)
      (if-let [slot (aget slots idx)]
        (let [cnt (aget slot-containers idx)
              att-lst (get slot-idx-att-lst idx)
              ]
          ;(.log js/console idx att-lst cnt)
          (doseq [att-nm att-lst]
            ;(.log js/console att-nm)
            (if-let [att (get-att idx att-nm)]
              (let [a 1
                    ;typ (.-type att)
                    ;renobj-name (get-internal-name att)
                    ]
                ;(.log js/console "phase1 " idx att-nm)
                (cond
                  (instance? region-class att)
                  (let [ar (.-region att)
                        sprite-name (.-name ar)]
                    (! slot.sprites (or (? slot.sprites) #js {}))
                    (if-let [sp (aget (? slot.sprites) sprite-name)]
                      (aset sp "_rndn_name" sprite-name)
                      (let [sp (.createSprite doll slot att sprite-name)]
                        (! sp.visible false)
                        ;(.log js/console "NEW SPRITE:" sprite-name)
                        (aset sp "_rndn_name" sprite-name)
                        (.addChild cnt sp))))
                  ;(or (= typ 2) (= typ 3))    ; mesh skinnedmesh
                  (instance? mesh-attch-class att)
                  (let [mesh-name (.-name att)]
                    (! slot.meshes (or (? slot.meshes) #js {}))
                    (if-let [m (aget (? slot.meshes) mesh-name)]
                      (aset m "_rndn_name" mesh-name)          ; do nothing
                      (let [m (.createMesh doll slot att)]
                        (! m.visible false)
                        ;(.log js/console "NEW MESH:" mesh-name)
                        (aset m "_rndn_name" mesh-name)
                        (.addChild cnt m))))
                  :else
                  (.log js/console "UNKNOWN TYPE:" att)
                  )))))))
    ...
    (doseq [sc slot-containers]
      ...
      (doseq [child (.-children sc)]
        ...
        (! child.visible false)
        ;(.log js/console (aget child "_rndn_name"))
        ))
    (mapv
      (fn [idx slt sc]
        (doseq [child (.-children sc)]
          ...
          (let [
            ...
            ;TODO char_113_cqbw_epoque_7 는 이게 null?
            ;
            (.log js/console child)
            (when (or (nil? c-nm) (empty? c-nm))
              ;(.warn js/console "slot" slt)
              (.warn js/console "child" child)
              )
            ...
            (aset layer "name" name)
            (when (and (> w 0) (> h 0))
            ;(when (or true (and (> w 0) (> h 0))) ; 항상 실행됨
    ...
    (if (or true js/window.chibi_psd_include_bones)
...
(defn gen-psd [doll nr hs]
  ...
  (let [
        shot-bg (or (? _h.bg)
                  (let [newbg (js/PIXI.Sprite.
                                js/PIXI.Texture.EMPTY)]
                    (! _h.bg newbg) newbg))
        ; 바운더리를 red-box 로 변경
        ;doll-bs (.getBounds doll)
    ...
    (let [
          ...
          ;draw-order (range 0 (count doll-cntr-children))
          ...
          filename (gstr/format "%s_%s_%s.psd" skin motion
                     (df/unparse dt-fmt (dt/time-now)))
          psd #js {:width     bg-width :height bg-height :channels 3 :bitPerChaneel 8
...
(defn gen-ar [atlas-text]
  (cond
    (and (some? js/PIXI.spine.TextureAtlasReader))
      (js/PIXI.spine.TextureAtlasReader. atlas-text)
    (and (some? js/PIXI.spine.core) (some? js/PIXI.spine.core.TextureAtlasReader))
      (js/PIXI.spine.core.TextureAtlasReader. atlas-text)
    ))
...
(defn onload-spine-data0 [key ldr rsc skin]
  (let [doll (gen-doll ldr rsc "Doll" skin)
        x (.val pos-x)
        y (.val pos-y)
        ]
...
;
    ...
    ;(if-let [b (js/_fs "hair_B2")]
    ;  (set! js/triangles (? b.attachment.triangles)) )

    ;_fs("hair_B2").attachment.triangles
...
(defn gen-loader []
  (cond
    (and (some? js/PIXI.loaders) (some? js/PIXI.loaders.Loader))
    (new js/PIXI.loaders.Loader)
    (and (some? js/PIXI.Loader))
    (new js/PIXI.Loader)
    ))
...
(defn init-core []
    ...
    (.on CLICK
      (fn [ev]
        (when (and js/hold_skel_json js/skel_json)
          (save-skel-json js/skel_json (str (? _h.doll.skin) "_c2g.json")))
        )))
      ...
      (fn [ev]
        (when (and js/hold_skel_json js/skel_json)
          (let [
                ...
                json2 (.parse js/JSON (.stringify js/JSON js/skel_json))
                version (? json2.skeleton.spine)
                [major minor] (str/split version ".")
                s33 (if (< (parse-int minor 3) 6) false true)
                no-dark (.prop (jQ "#no-slot-dark") "checked")
                ]
            (features/cv4spine38 json2 s33 no-dark)
          (save-skel-json json2 filename))
        )
        )))
  ...
  ;  (.on CLICK
  ;    (fn [ev]
  ;      (let [n (first (filter #(re-find #"\.atlas" %) (js-keys (? _h.urls))))
  ;         u (aget (? _h.urls) n)]
  ;      (features/gen-4x n u))
  ;      )))
  ...
  (-> (jQ "#unpackTextureBtn")
    ...
    (.on CLICK
      (fn [ev]
        ...
        (js/alert
          (str "the function is currently disabled because of unknown errors"
        "\nlet me have some time to fix it"))
        ;(if js/sd_atlas
  ...
  ;(init-debug)
  ;(set! js/reload init-debug)
  ;(-> (jQ "#reloadBtn")scale_xy
  ;  (.off)
  ;  (.on CLICK
  ;    (fn [ev]
  ;      (init-debug)
...
;(defn init-debug []
;  (load-spine-data-url3
;    "./spine/az/22.atlas"
;    "./spine/az/22.png"
;    "./spine/az/22_db_4.json"
;    "22"
;    0)
;  nil)
...
;(defn init-debug []
;  (load-spine-data-url3
;    "./spine/ak/char_002_amiya.atlas"
;    "./spine/ak/char_002_amiya.png"
;    ;"./spine/ak/char_002_amiya.skel"
;    ;"./spine/ak/char_002_amiya_c2g.json"
;    "./spine/ak/char_002_amiya_db_7.json"
;    ;"./spine/ak/char_002_amiya_db.json"
;    ;"./spine/prknr/105811.atlas"
;    ;"./spine/prknr/105811.png"
;    ;"./spine/prknr/105811.skel"
;    "char_002_amiya"
;    1
;    )
;  nil)
...
;(defn init-debug2 []
;  (load-spine-data-url3
;    "./spine/ak/char_009_12fce.atlas"
;    "./spine/ak/char_009_12fce.png"
;    "./spine/ak/char_009_12fce.skel"
;    "char_009_12fce"
;    false
;    )
;  nil)

;(set! js/ak_debug init-debug)
;(set! js/ak_debug2 init-debug2)
