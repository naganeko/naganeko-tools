function SkeletonBinary3() {
};

SkeletonBinary3.prototype = {
  data: null,
  scale: 1,
  json: {},
  nextNum: 0,
  ...
  readString_off() {
    ...
    var t = this.readInt(this, true);
    if (0 == t) return null;
    if (0 == t) return "";

    var e = new Uint8Array(this.data.slice(this.nextNum, this.nextNum + t - 1));
    // return this.offset += t - 1,
    this.nextNum += t - 1;
    console.log(e);
    // s = decodeURIComponent(escape(String.fromCharCode.apply(null, e)));
    s = String.fromCharCode.apply(null, e);
    console.log(s);
    return s;
  ...
  readString() {
    ...
    // console.log("nextNum=", this.nextNum, "c=", charCount);
    switch (charCount) {
      case 0:
        return null;
      case 1:
  ...
  initJson: function () {
    ...
    console.assert(skeleton.spine.startsWith("3.5"), "This parser supports only 3.5.x file format");

    window.sp35data = this.data;
    ...
    console.log("bones len=", bones.length);
    window.s35bones_ni = {};
    window.s35bones_in = [];
    for (var i = 0; i < bones.length; i++) {
      let b = bones[i];
      console.log(i, b);
      window.s35bones_ni[b.name] = i;
      // window.s35bones_ni[bones[i]] = i;
      window.s35bones_in[i] = b.name;
    }
    ...
    for (var i = 0; i < slots.length; i++) {
      ...
      let bIdx = this.readInt(true);
      console.log(slotData.name,slotData.name.length,"b=" ,bones[bIdx].name , "bIdx=", bIdx);
      var boneData = this.json.bones[bIdx];
      let bone_name = boneData.name;
      slotData.bone = bone_name;
      ...
      // slotData.blend = BlendMode[this.readByte(true)];
      slots[i] = slotData;
    }
