...
(defn out-of-area? [bs wp-bs]
  (let [x (.-x bs) y (.-y bs)
        w (.-width bs) h (.-height bs)
        w2 (/ w 2) h2 (/ h 2)
        wpx (.-x wp-bs)
        wpy (.-y wp-bs)
        wpw (.-width wp-bs)
    ...
    (doseq [ir (range 0 (.-length data) 4)]
      (let [ig (+ 1 ir)
        ...
        (when (< a 255)
          ...
          (aset data ig (f g a))
          (aset data ib (f b a))
          ;(aset data ir (min 255 (Math/round (op r (/ a 255.0)))))
          ;(aset data ig (min 255 (Math/round (op g (/ a 255.0)))))
          ;(aset data ib (min 255 (Math/round (op b (/ a 255.0)))))
          )))
...
(defn unpma [png-data]
  (let [w (.-width png-data)
        h (.-height png-data)
        canvas js/window.png_wrk_canvas
