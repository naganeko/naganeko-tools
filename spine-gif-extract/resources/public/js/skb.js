/*
skb.js
Spine 2.1 Skeleton Binary (.skel) reader

comment from naganeko
Hello, I have no idea who made this converter at first.
if you have some requests about this program,
contact @lazy_schemer (twitter)

CHANGES
2022-02-20
FIXED : reading animation ffd timeline
FIXED : reading skin names
...
SkeletonBinary.prototype = {
  ...
  initJson: function () {
    ...
    if (defaultSkin != null) {
      ...
      console.log("default skin NOT FOUND");
    }
...
//      console.log("skinName=>" , skinName, "<");
    ...
    if (!window.DEBUG_SKIP_ANIMATIONS) {
      for (var i = 0, n = this.readInt(true); i < n; i++) {
...
//    console.log("slotCount=", slotCount);
  ...
  readAttachment: function (attachmentName, nonessential) {
    var name = this.readString();
//    console.log("att name=", attachmentName, name)
    if (name == null)
      name = attachmentName;
   //   AttachmentType: ["region", "boundingbox", "mesh", "skinnedmesh"]
    var ty = this.AttachmentType[this.readByte()];
    // console.log(attachmentName, name, ty);
    switch (ty) {
      case "region":
  ...
  readAnimation: function (name) {
    ...
    var ik = {};
    {
      ...
      for (var i = 0, n = n0; i < n; i++) {
        ...
        let key0 = this.json.ik[ikIndex].name;
        ik[key0] = timeline;
        console.log("ik=", key0 , "=>", timeline);
      }

      console.log("this.json.ik=", this.json.ik);
      console.log("ik=", ik);
    }
    animation.ik = ik;
...
//        console.log("ffd slot ", slot.name , slot);
...
//          console.log("meshName=>" + meshName + "<");
          ...
          // 2022-02-20 skin의 attchment명이 atlas의 obj명과 다를 수도 있음
          if (!attachment) {
            for (var attachmentName in attachments) {
              // console.log(attachmentName
              //   , attachments[attachmentName].name, meshName, attachments[attachmentName].name == meshName);
          ...
          for (var frameIndex = 0; frameIndex < frameCount; frameIndex++) {
            ...
            if (attachment.type == "mesh") {
              ...
              // 이게 왜 되는지는 모르겠음
              // console.warn("NOT mesh=", attachment);
